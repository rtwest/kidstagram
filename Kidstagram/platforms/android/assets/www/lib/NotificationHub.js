(function () {
    ...
    NotificationHub = function (client) {
    ...
    // This gets a registrationID from the NotificationHub to use in registration
    var createRegistrationId = function (hub) {

        document.getElementById('log').innerHTML += 'running createRegistrationId</br>'// old school dom injection
        ...
        // XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
        //  The bug is in here below
        // XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
        // XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX


        hub.client._request(method, uriFragment, null, null, null, function (error, response) {
            if (error) {
                ...
                document.getElementById('log').innerHTML += 'notificationHub:azureClient:request response location parsed RegID from NotHub - ' + regId + '</br>'// old school dom injection
    ...
    // You have to use the RegistrationID asked for above to register a device 
    var updateRegistration = function (hub, regId, registration) {

        document.getElementById('log').innerHTML += 'running updateRegistration</br>'// old school dom injection
        ...
        hub.client._request(method, uriFragment, registrationPayload, null,
            null, function (error, response) {
            if (error) {
                console.log("Error: " + error);
                deferred.reject("Error: " + error);
                document.getElementById('log').innerHTML += 'notificationHub:azureClient:updateNotification error - ' + error + '</br>'// old school dom injection
    ...
    var deleteRegistration = function (hub, regId) {
        ...
        // Send a Notification Hubs registration update to the Mobile Service.
        hub.client._request(method, uriFragment, null, null,
            null, function (error, response) {                
                if (error) {
                    console.log("Error: " + error);
                    deferred.reject("Error: " + error);
    ...
    // This is where the get RegistrationID then register with it are put together
    ...
    var buildRegKey = function (hub, registration) {
        ...
        // create key
        var regKey = hub.client.applicationUrl + tileKey;

        if (registration.templateName) {
            regKey = regKey + '/' + registration.templateName;
        }
        return regKey;
        document.getElementById('log').innerHTML += 'NotificationHub: Build regKey - ' + regKey + ' </br>'// old school dom injection
    ...
    var storeInContainer = function (key, object) {
        ...
        document.getElementById('log').innerHTML += 'NotificationHub: Saved regID and deviceToken</br>'// old school dom injection
    ...
    var getFromContainer = function (key) {
        ...
        if (typeof localStorage.getItem(key) === 'string') {
            ...
            document.getElementById('log').innerHTML += 'NotificationHub: Retrieved regID and deviceToken</br>'// old school dom injection
        ...
        if (typeof regInfo !== 'undefined') {
            ...
            updateRegistration(hub, regInfo.regId, registration).then(
                function () {
                    ...
                    document.getElementById('log').innerHTML += 'NotificationHub: Updated regInfo</br>'// old school dom injection
            ...
            return createRegistration(hub, registration).then(
                function (regId) {
                    ...
                    document.getElementById('log').innerHTML += 'NotificationHub: Created new registration</br>'// old school dom injection
